...
function WalletPage() {
		...
		if (userId) {
			try {
				const response = await axios.get(
					`https://api.devask.hng.tech/user/wallet/view/${userId}`
				);
	...
	const fetchTransactionHistory = async () => {
		if (userId) {
			try {
				const response = await axios.get(
					`https://api.devask.hng.tech/admin/transactions/users/${userId}?skip=0&limit=30`
				);
				...
				<section className={styles.firstSection}>
					...
					<div className={styles.recentHistoryWrapper}>
						...
						<div className={styles.recentHistoryBlock}>
							<button
								onClick={handleEarnedTransaction}
								className={`${styles.historyButtons1} ${
									earned && styles.currentButton
								}`}
								type="button"
							>
								Earned
							...
							<button
								onClick={handleSpentTransaction}
								className={`${styles.historyButtons2} ${
									!earned && styles.currentButton
								}`}
								type="button"
							>
							...
							{earned
								? data.transactionHistory.slice(0, 7).map(
										(userTransaction) =>
											userTransaction.transacion_type.value === 'Earned' && (
												<div
													key={userTransaction.transaction_id}
													className={styles.transactionBlock}
												>
													<h4>{`${
														month[getMonth(userTransaction.transaction_date)]
													} ${getFullYear(
														userTransaction.transaction_date
													)}`}</h4>
													<div className={styles.transaction}>
														<img src={transaction} alt="transactionMade" />
														<p>
															You’ve successfully earned tokens for a correct
															reply
														</p>
														<div className={styles.recentFlex}>
															<span className={styles.date}>{`${getFullYear(
																userTransaction.transaction_date
															)}-${
																getMonth(userTransaction.transaction_date) + 1
															}-0${getDay(
																userTransaction.transaction_date
															)}`}</span>
															<span className={styles.token}>
																+{userTransaction.amount} tokens
															</span>
														</div>
													</div>
												</div>
											...
											userTransaction.transacion_type.value === 'Spent' && (
												...
												>
													...
													<div className={styles.transaction}>
														<img src={transaction} alt="transactionMade" />
														<p>
															You’ve successfully spent tokens for a question
														</p>
														<div className={styles.recentFlex}>
															<span className={styles.date}>{`${getFullYear(
																userTransaction.transaction_date
					...
					<div className={styles.transactionHistoryWrapper}>
						<div className={styles.transactionHistoryHeader}>
							...
							>
								...
								{viewAll ? (
									<img src={viewMore} alt="view all" />
								) : (
									<img src={viewLess} alt="view less" />
								)}
							</span>
						</div>
							...
							{viewAll
										...
										userTransaction.transacion_type.value === 'Earned' ? (
												...
												<div className={styles.transactionHistoryBlock}>
													...
													<div className={styles.transactionHistoryRecord}>
														<p className={styles.p1}>
															{userTransaction.amount} token Deposited
															Successfully
														</p>
														<p className={styles.p2}>{data.walletId}</p>
													</div>
													<span>
														{getTime(userTransaction.transaction_date)}
													</span>
												</div>
											</div>
										) : (
											<div key={userTransaction.transaction_id}>
												...
												<div className={styles.transactionHistoryBlock}>
													...
													<div className={styles.transactionHistoryRecord}>
														<p className={styles.p1}>
															{userTransaction.amount} token Withdrawn
															Successfully
														</p>
														<p className={styles.p2}>{data.walletId}</p>
													</div>
								...
								: data.transactionHistory.map((userTransaction) =>
										userTransaction.transacion_type.value === 'Earned' ? (
											<div
												key={userTransaction.transaction_id}
												className={styles.transactionFlex}
											>
												<h4>{`${
													month[getMonth(userTransaction.transaction_date)]
												} ${getFullYear(
